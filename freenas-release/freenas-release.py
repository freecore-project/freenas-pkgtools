...
class SQLiteReleaseDB(object):
    ...
    def AddRelease(self, manifest):
        ...
        if manifest.Notes():
            for note in list(manifest.Notes().keys()):
                sql = """
...
def AddPackageUpdateScript(db, archive, pkg, name, script, lock = True):
    ...
    """
    print("AddPackageUpdateScript(db, %s, %s-%s, %s, %s, lock = %s)" % (archive, pkg.Name(), pkg.Version(), name, script, lock), file=sys.stderr)
        ...
        if archive:
            ...
            if package_services:
                if "Restart" in package_services:
                    ...
                    tlist = []
                    for svc in list(pkg_restart_list.keys()):
                        tlist.append(svc)
                    if tlist:
...
def Check(archive, db, project = "FreeNAS", args = []):
    ...
    for t in list(sequences.keys()):
        ...
        if expected_contents != found_contents:
            ...
            for seq in list(expected_contents.keys()):
    ...
    if (quick and list(expected_packages.keys()) != list(found_packages.keys())) or \
       (quick is False and expected_packages != found_packages):
        ...
        for expected in list(expected_packages.keys()):
        ...
        for found in list(found_packages.keys()):
...
#            print >> sys.stderr, "Found Notes: %s" % found_notes
        for n in list(found_notes.keys()):
